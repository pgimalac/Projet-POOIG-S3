...
public class Fenetre extends JFrame{
	...
	/* PAGE DE PARAMETRES */
		...
		parametres.add(boite,new GridBagConstraints());
			/* IDEES DE PARAMETRES
					- checkbox : afficher les positions des pions de chaque joueur à la fin de chaque tour
					- checkbox : afficher les informations de jeu dès qu'un joueur joue ("joueur fait .. et arrive sur la case ..")
					- checkbox : afficher quand un joueur ne peut pas jouer 
					- radioButton : choisir le type d'affichage
			*/
		position=false;
		JCheckBox opt=new JCheckBox("Afficher un message contenant la position des pions de chaque joueur à la fin de chaque tour.",position);
		...
		mouvement=false;
		opt=new JCheckBox("Afficher un message contenant les informations du mouvement dès qu'un joueur joue.",mouvement);
		...
		cantPlay=false;
		opt=new JCheckBox("Afficher un message quand un joueur ne peut pas jouer.",cantPlay);
				...
				public void goTo(){
					...
					for (String s:t){
						if (s.matches(Affichage.REGEX_SAVE)){
							boite.add(
								Box.createHorizontalBox().add(
									new ButtonUp(i+". "+t[i-1],true,true,event -> {
										affichage.setJeu(affichage.chargerLeJeu(s));
					...
					boite.add(new ButtonUp("MENU",true,true, event -> fireGoToMenu()));
				}
			};
		}else{
			charger=null;
	...
	public void fireGoToCharger(){ affiche=charger; charger.goTo(); if (affiche==charger) cardLayout.show(content,"charger"); }
